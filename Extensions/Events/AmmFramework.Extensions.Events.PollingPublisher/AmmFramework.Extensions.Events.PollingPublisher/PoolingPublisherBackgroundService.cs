...
namespace AmmFramework.Extensions.Events.PollingPublisher;
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                foreach (var item in outboxItems)
                {
                    ...
                    _logger.LogDebug("event {eventName} with {EventId} sent from {ApplicationName} at {DateTime}", item.EventName, item.EventId, _options.ApplicationName, DateTime.Now);
                }
                _outBoxEventItemRepository.MarkAsRead(outboxItems);
                if (outboxItems.Any())
                {
                    _logger.LogInformation("{Count} events {ApplicationName} at {DateTime} with id {Ids}", outboxItems.Count, _options.ApplicationName, DateTime.Now, string.Join(',', outboxItems.Select(c => c.EventId)));
                }
                await Task.Delay(_options.SendInterval, stoppingToken);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Exception acquired in PoolingPublisherBackgroundService for application {ApplicationName}", _options.ApplicationName);
